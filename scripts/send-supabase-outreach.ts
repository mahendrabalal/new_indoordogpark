...
async function main() {
    console.log('🚀 Starting Supabase Outreach Campaign...');
    ...
    // 3. Fetch Approved Parks
    console.log('📦 Fetching approved parks from Supabase...');
    ...
    if (!parks || parks.length === 0) {
        console.log('⚠️ No approved parks found to email.');
        process.exit(0);
    }
    ...
    for (const park of parks) {
        if (!park.email) {
            console.log(`⏭️  Skipping ${park.name} (no email)`);
            results.push({ park: park.name, status: 'skipped', reason: 'no email' });
            continue;
        }

        console.log(`📧 Sending to ${park.name} (${park.email})...`);
        ...
        try {
            ...
            if (result.error) {
                ...
                failCount++;
                results.push({ park: park.name, email: park.email, status: 'failed', error: result.error });
            } else {
                console.log(`   ✅ Success (ID: ${result.data.id})`);
                successCount++;
                results.push({ park: park.name, email: park.email, status: 'sent', id: result.data.id });
            }

        } catch (err: any) {
            console.error(`   ❌ Exception: ${err.message}`);
            failCount++;
            results.push({ park: park.name, email: park.email, status: 'error', error: err.message });
        }
    ...
    // Save log
    const logFile = `outreach-log-${Date.now()}.json`;
