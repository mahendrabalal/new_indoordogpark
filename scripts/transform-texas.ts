...
const sourcePath = path.join(process.cwd(), 'public/data/texas.json');
const backupPath = path.join(process.cwd(), 'public/data/texas.json.bak');
...
const targetParks: TargetPark[] = sourceParks.map(source => {
    ...
    const description = source.description || source.scrapedDescription || `Dog park in ${source.city}, TX`;
    ...
    return {
        ...
        city: source.city || '',
        state: 'TX', // Normalize to TX
        zipCode: source.postal_code || '',
        full_address: source.full_address || `${source.street}, ${source.city}, TX ${source.postal_code}`,
...
console.log(`Transformed ${targetParks.length} parks in texas.json`);
