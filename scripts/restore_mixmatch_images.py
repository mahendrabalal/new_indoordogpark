#!/usr/bin/env python3
"""
Restore images for mixmatch.json parks from Google Places API.
Fetches place details for parks and updates their photos array.
"""
...
def restore_images_for_parks(api_key: str, parks: List[Dict[str, Any]]) -> tuple[List[Dict[str, Any]], int]:
    """Restore images for parks from Google Places API."""
    updated_parks = []
    restored_count = 0
    ...
    for i, park in enumerate(parks, 1):
        ...
        photo_objects = get_photo_urls(api_key, photos, max_photos=5)
    ...
    return updated_parks, restored_count

def main():
    """Main function to restore images for mixmatch.json parks."""
    import sys
    import os
    ...
    if not api_key:
        ...
        print("  1. As command line argument: python3 scripts/restore_mixmatch_images.py YOUR_API_KEY")
    ...
    backup_path = Path('public/data/mixmatch.json.backup_images')
    ...
    # Restore images
    updated_parks, restored_count = restore_images_for_parks(api_key, parks)
