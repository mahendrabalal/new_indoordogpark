...
const sourcePath = path.join(process.cwd(), 'public/data/pennsylvania.json');
const backupPath = path.join(process.cwd(), 'public/data/pennsylvania.json.bak');
...
const targetParks: TargetPark[] = sourceParks.map(source => {
    ...
    const description = source.description || source.scrapedDescription || `Dog park in ${source.city}, PA`;

    // Determine Business Type
    let businessType = 'Dog Park'; // Default
    if (source.type === 'Dog facility' || source.category === 'Dog facility' || source.category === 'Dog day care center' || source.type === 'Dog day care center') {
    ...
    return {
        ...
        city: source.city || '',
        state: 'PA', // Normalize to PA
        zipCode: source.postal_code || '',
        full_address: source.full_address || `${source.street}, ${source.city}, PA ${source.postal_code}`,
...
console.log(`Transformed ${targetParks.length} parks in pennsylvania.json`);
