#!/usr/bin/env python3
"""
Restore missing images for Washington parks from Google Places API.
Fetches place details for parks missing images and updates washington.json.
"""
...
def load_api_key(env_path: Path = Path(".env"), env_var: str = "google_place_api") -> str:
    ...
    env_files = [env_path, Path(".env.local"), Path(".env"), Path("../.env"), Path("../../.env")]
...
def get_photo_urls(api_key: str, photos: List[Dict[str, Any]], max_photos: int = 5) -> List[str]:
    """Convert photo references to downloadable URLs."""
    if not photos:
        return []
...
def restore_images_for_parks(api_key: str, parks: List[Dict[str, Any]]) -> tuple[List[Dict[str, Any]], int, int]:
    """Restore images for parks missing them. Downloads images locally."""
    updated_parks = []
    restored_count = 0
    downloaded_count = 0
    
    # Find parks that need images: either no photos in JSON, or photos exist in JSON but files don't exist
    parks_missing_images = []
    for park in parks:
        ...
        if needs_images:
            parks_missing_images.append(park)
    
    print(f"\nFound {len(parks_missing_images)} parks missing images")
    print("=" * 80)
    
    for i, park in enumerate(parks_missing_images, 1):
        ...
        photo_urls = get_photo_urls(api_key, photos, max_photos=3)
        ...
        print(f"Downloading {len(photo_urls)} images...", end=" ")
        ...
        if downloaded_photos:
            ...
            updated_park["photos"] = downloaded_photos
            updated_park["photo"] = downloaded_photos[0]["url"]  # Set primary photo
            updated_parks.append(updated_park)
            restored_count += 1
            print(f"SUCCESS ({len(downloaded_photos)} images downloaded)")
        else:
    ...
    # Add parks that already have images (unchanged)
    parks_with_images = [
        park for park in parks 
        if park.get("photos") and len(park.get("photos", [])) > 0
    ]
    
    all_parks = updated_parks + parks_with_images
    
    return all_parks, restored_count, downloaded_count

def main():
    """Main function to restore images for Washington parks."""
    import sys
    import os
    ...
    if not api_key:
        try:
            ...
            print(f"✓ Loaded Google Places API key from file")
        except Exception as e:
    ...
    if not api_key:
        ...
        print("  1. As command line argument: python3 scripts/restore_washington_images.py YOUR_API_KEY")
        print("  2. As environment variable: export google_place_api=YOUR_API_KEY")
        print("  3. In .env or .env.local file: google_place_api=YOUR_API_KEY")
    ...
    backup_path = Path('public/data/washington.json.backup_images')
    ...
    # Restore images
    updated_parks, restored_count, downloaded_count = restore_images_for_parks(api_key, parks)
    ...
    print(f"  Parks with images before: {len([p for p in parks if p.get('photos')])}")
